    ...
    public function render($path, $compress = FALSE) {
        ...
        if (TYPO3_MODE === 'FE') {
            ...
            if (strtolower(substr($path, -3)) === '.js') {
                $GLOBALS['TSFE']->getPageRenderer()->addJsFooterFile($path, NULL, $compress, FALSE);

                // CSS
            } elseif (strtolower(substr($path, -4)) === '.css') {
                $GLOBALS['TSFE']->getPageRenderer()->addCssFile($path, 'stylesheet', 'all', '', $compress);
            } elseif (preg_match($searchpattern_1, $path) && preg_match($searchpattern_2, $path)) {
                $GLOBALS['TSFE']->getPageRenderer()->addJsFile($path, NULL, $compress, FALSE);
            }
        } else {
