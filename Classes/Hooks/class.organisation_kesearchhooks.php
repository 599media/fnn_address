 ...
 * It implements a simple indexer for fnn_address_organisations.
 ***************************************************************/

class organisation_kesearchhooks {
	...
	function registerIndexerConfiguration(&$params, $pObj) {
		...
		$newArray = array(
				...
				'fnnaddressorganisations',
				TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extRelPath('fnn_address') . 'Resources/Public/Icons/fnnaddressorganisations.gif'
				);
		$params['items'][] = $newArray;

			// enable "sysfolder" field
		$GLOBALS['TCA']['tx_kesearch_indexerconfig']['columns']['sysfolder']['displayCond'] .= ',fnnaddressorganisations';
	}
	...
	public function customIndexer(&$indexerConfig, &$indexerObject) {
		if($indexerConfig['type'] == 'fnnaddressorganisations') {
			$content = '';
			...
			if($resCount) {
				while ( ($record = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) ) {
                    ...
                    $alternativeName = strip_tags($record['alternative_name']);
					$abstract = strip_tags($record['name']);
					$fullContent = $name . "\n" . $alternativeName . "\n" . $content;
					$params = '&tx_fnnaddress_displayorganisations[organisation]=' . $record['uid'] . '&tx_fnnaddress_displayorganisations[action]=show';
...
//					$title = '[ELEMENT INDEXER] ' . $title;
					...
					$indexerObject->storeInIndex(
                            ...
                            $name,                          // record title
                            'fnnaddressorganisations',      // content type
