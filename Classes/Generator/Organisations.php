...
class Organisations implements \Tx_ExtensibleSitemap_Generator_Interface {
     ...
     * @var string
     */
    protected $fieldList = 'uid,pid,name,tstamp';
    ...
    protected function createHandle() {
        ...
        $selectWhere =
            (empty($pids) ? '1=1' : 'pid IN (' . $pids . ')').
                ...
                $this->cObj->enableFields('tx_fnnaddress_domain_model_organisation')
        ;
//        \TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($this->fieldList);
        $this->handle = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
            $this->fieldList,
            'tx_fnnaddress_domain_model_organisation',
            $selectWhere,
            ''
        );
    }
...
//            \TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($this->processItem($news));
    ...
    protected function processItem($item) {
        ...
        $href = $this->cObj->typoLink_URL(array(
            ...
            'additionalParams' => sprintf(
                '&tx_fnnaddress_displayorganisations[organisation]=%d&tx_fnnaddress_displayorganisations[action]=show&tx_fnnaddress_displayorganisations[controller]=Organisation',
                $item['uid']
            ),
            'useCacheHash' => true
        ));
